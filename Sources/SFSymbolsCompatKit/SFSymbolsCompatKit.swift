...
public extension UIImage {
    ...
    convenience init?(systemName name: String, withConfiguration config: SymbolConfigurationA? = nil) {
        ...
        // Partial padding reduction: keep ~70% of original ascender/descender space
        let topPadding = font.ascender - tightRect.maxY
        let bottomPadding = tightRect.minY - font.descender
        let reductionFactor: CGFloat = 0.7

        let adjustedRect = CGRect(
            x: tightRect.origin.x,
            y: tightRect.origin.y + bottomPadding * (1 - reductionFactor),
            width: tightRect.width,
            height: tightRect.height + (topPadding + bottomPadding) * (1 - reductionFactor)
        )

        // Render the symbol
        let imageSize = adjustedRect.size
        UIGraphicsBeginImageContextWithOptions(imageSize, false, 0)
        attrString.draw(at: CGPoint(x: -adjustedRect.origin.x, y: -adjustedRect.origin.y))
